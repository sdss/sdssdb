...
def scrapeRecent(obs="APO", mjd_today=00000, base="", 
    ...
    regex = re.compile(rf'\bMJD {int(mjd_today)}\b')

    for tdir in top_level:
        thisdir = os.path.join(obs_dir, tdir)
        sub_dirs = os.listdir(thisdir)

        check_mid_level.extend([os.path.join(thisdir, sdir) for sdir in sub_dirs])
        # for sdir in sub_dirs:
        #     subdir = os.path.join(thisdir, sdir)
        #     timeStamp = os.path.getmtime(subdir)
        #     if timeStamp > use_unix:
        #         continue

        #     if (use_unix - timeStamp) / secInDay < n_days and os.path.isdir(subdir):
    ...
    for sub in check_mid_level:
        ...
        for s in sum_files:
            ...
            # timeStamp = os.path.getmtime(sum_name)
            # if (use_unix - timeStamp) / secInDay > n_days or \
            #     timeStamp > use_unix:
            #     continue
            ...
            try:
                ...
                continue
            if design_id not in doneDesigns or design_id < 0 or is_dithered:
                continue
...
def updateDB(obs="APO", badTargsFile=""):
    ...
    if obs == "APO":
        dbVersion = targetdb.Version.get(plan="eta-6")
    else:
        dbVersion = targetdb.Version.get(plan="eta-7")
...
def cpOpsdbToStat(mjd, obs):
    ...
    where d2s.mjd > {mjd-0.2} and d2s.mjd < {mjd + 1}  and d2s.completion_status_pk = 3
    )
...
if __name__ == "__main__":
    ...
    if mjd is not None:
        mjd_now = mjd
        # default check is 2 days back, so use_unix 1 day ahead catches all
        use_unix = Time(mjd + 1, format="mjd").unix
