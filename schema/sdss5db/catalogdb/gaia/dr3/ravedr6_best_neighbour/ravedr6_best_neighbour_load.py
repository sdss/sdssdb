# Program: ravedr6_best_neighbour_load.py
# Aim: load ravedr6_best_neighbour csv files into the 
#      postgreSQL sdss5db database.
#
# The program checks so that it does not reload csv files which already
# have a csv.load.out file.
# run ravedr6_best_neighbour_csv2csv.py before running this program
...
csv_dir = "/uufs/chpc.utah.edu/common/home/sdss50/sdsswork/target/catalogs/gaia/dr3/ravedr6_best_neighbour/csvout/"  # noqa E501
...
list_of_csv_files = glob.glob(csv_dir + "Ravedr6BestNeighbour*nometa.csv")
...
for i in range(len(list_of_csv_files)):
    ...
    print("\\copy catalogdb.gaia_dr3_ravedr6_best_neighbour ", file=fpgscript, end='')
