# Program: qso_candidates_csv2csv.py
# Aim: Do below changes to Gaia DR3 qso_candidates CSV files
#      The original CSV files are not modified.
#      The modified files have the extension nometa.csv.
#      (1) remove 273 lines of metadata at beginning of the file.
#
# sed -i -e '1,273d' temp1.csv
...
csv_dir = "/uufs/chpc.utah.edu/common/home/sdss50/sdsswork/target/catalogs/gaia/dr3/qso_candidates/"  # noqa E501
csvout_dir = "/uufs/chpc.utah.edu/common/home/sdss50/sdsswork/target/catalogs/gaia/dr3/qso_candidates/csvout/"  # noqa E501

list_of_csv_files = glob.glob(csv_dir + "QsoCandidates*.csv")
list_of_csv_files.sort()

fout = open(csvout_dir + "nometa.csv.out", "a")

if DEBUG is True:
    list_of_csv_files = [csv_dir + "QsoCandidates_000000-003111.csv"]
...
for i in range(len(list_of_csv_files)):
    ...
    # We use use '1,273d' so that the first 273 lines are deleted
    # These deleted lines contain metadata about the file.
    # We do not delete the header line in the CSV file.
    print("sed  -e '1,273d' " + full_csv_file + " > " + full_csv_nometa_file, file=fpgscript)
