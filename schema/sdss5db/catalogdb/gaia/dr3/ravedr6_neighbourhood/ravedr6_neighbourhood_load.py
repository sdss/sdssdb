# Program: ravedr6_neighbourhood_load.py
# Aim: load ravedr6_neighbourhood csv files into the 
#      postgreSQL sdss5db database.
#
# The program checks so that it does not reload csv files which already
# have a csv.load.out file.
# run ravedr6_neighbourhood_csv2csv.py before running this program
...
csv_dir = "/uufs/chpc.utah.edu/common/home/sdss50/sdsswork/target/catalogs/gaia/dr3/ravedr6_neighbourhood/csvout/"  # noqa E501
...
list_of_csv_files = glob.glob(csv_dir + "Ravedr6Neighbourhood*nometa.csv")
list_of_csv_files.sort()

fout = open(csvout_dir + "load.csv.out", "a")

if DEBUG is True:
    list_of_csv_files = [csv_dir + "Ravedr6Neighbourhood.nometa.csv"]
...
for i in range(len(list_of_csv_files)):
    ...
    print("\\copy catalogdb.gaia_dr3_ravedr6_neighbourhood ", file=fpgscript, end='')
