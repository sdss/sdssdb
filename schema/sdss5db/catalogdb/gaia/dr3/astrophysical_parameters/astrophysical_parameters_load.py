# Program: astrophysical_parameters_load.py
# Aim: load astrophysical parameters csv files into the 
#      postgreSQL sdss5db database.
#
# The program checks so that it does not reload csv files which already
# have a csv.load.out file.
# run astrophysical_parameters_csv2csv.py before running this program
...
csv_dir = "/uufs/chpc.utah.edu/common/home/sdss50/sdsswork/target/catalogs/gaia/dr3/astrophysical_parameters/csvout/"  # noqa E501
...
list_of_csv_files = glob.glob(csv_dir + "AstrophysicalParameters*nometa.csv")
list_of_csv_files.sort()

fout = open(csvout_dir + "load.csv.out", "a")

if DEBUG is True:
    list_of_csv_files = [csv_dir + "AstrophysicalParameters_000000-003111.nometa.csv"]
...
for i in range(len(list_of_csv_files)):
    ...
    print("\\copy catalogdb.gaia_dr3_astrophysical_parameters ", file=fpgscript, end='')
