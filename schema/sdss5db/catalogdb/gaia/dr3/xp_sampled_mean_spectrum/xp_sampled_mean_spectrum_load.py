# Program: xp_sampled_mean_spectrum_load.py
# Aim: load xp_sampled_mean_spectrum csv files into the 
#      postgreSQL sdss5db database.
#
# The program checks so that it does not reload csv files which already
# have a csv.load.out file.
# run xp_sampled_mean_spectrum_csv2csv.py before running this program
...
csv_dir = "/uufs/chpc.utah.edu/common/home/sdss50/sdsswork/target/catalogs/gaia/dr3/xp_sampled_mean_spectrum/csvout/"  # noqa E501
...
list_of_csv_files = glob.glob(csv_dir + "XpSampledMeanSpectrum*nometa.csv")
list_of_csv_files.sort()

fout = open(csvout_dir + "load.csv.out", "a")

if DEBUG is True:
    list_of_csv_files = [csv_dir + "XpSampledMeanSpectrum_000000-003111.nometa.csv"]
...
for i in range(len(list_of_csv_files)):
    ...
    print("\\copy catalogdb.gaia_dr3_xp_sampled_mean_spectrum ", file=fpgscript, end='')
