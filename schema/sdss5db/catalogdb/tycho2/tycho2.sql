...
The Tycho-2 catalogue including the supplemental table 1.

/uufs/chpc.utah.edu/common/home/sdss10/sdss5/target/catalogs/tycho2/Tycho-2

Downloaded from ftp://cdsarc.u-strasbg.fr/pub/cats/I/259/

Note 1: we add an id_tycho column defined as in the Gaia cross-matching
    (http://bit.ly/2TZD4Eg): (TYC1*1000000.0d0)+(TYC2*10.0d0)+(TYC3*1.0d0).

Note 2: The columns HIP and CCDM are missing the delimiter in the files. This
    is fixed in the loading process.
...
This script loads the file tycho2_all.csv that was generated by loading
the tyc2.dat and suplement files using
astropy.io.ascii.read(file, format='csv', readme='./ReadMe') and then concatenating
them with astropy.table.vstack. The resulting table was then saved as CSV.
...
ALTER TABLE catalogdb.tycho2 ADD COLUMN pk BIGSERIAL;
DELETE FROM catalogdb.tycho2 a USING catalogdb.tycho2 b
    WHERE a.designation = b.designation AND a.pk > b.pk;
ALTER TABLE catalogdb.tycho2 DROP COLUMN pk;
