    ...
    def add_table_constraints(cls, constraints):
        ...
        if cls.__table__.columns.keys():
            for con in constraints:
                assert isinstance(con, ForeignKeyConstraint), (
                    "constraint must be a ForeignKeyConstraint"
                )
                cls.__table__.append_constraint(con)


# The softening parameter for asinh magnitudes
bb = {"u": 1.4e-10, "g": 0.9e-10, "r": 1.2e-10, "i": 1.8e-10, "z": 7.4e-10}
...
class CurrentCatalogue(Base):
    ...
    catalogue = relationship("Catalogue", backref=backref("currentCatalogue", uselist=False))
...
def HybridMag(flux_parameter, band, index=None):
    ...
    def hybridMag(self):
        ...
        bb_band = bb[band]
        asinh_mag = -2.5 / np.log(10) * (np.arcsinh(flux / (2.0 * bb_band)) + np.log(bb_band))
        return asinh_mag
...
def HybridColour(parameter):
    ...
    def colour(self, bandA, bandB):
        for band in [bandA, bandB]:
            ...
            assert hasattr(self, columnName), "cannot find column {0}".format(columnName)
    ...
    def colour(cls, bandA, bandB):
        for band in [bandA, bandB]:
            columnName = parameter + "_" + band
            assert hasattr(cls, columnName), "cannot find column {0}".format(columnName)
...
for colour_a, colour_b in itertools.combinations("ugriz", 2):
    setattr(
        NSA,
        "elpetro_mag_{0}_{1}".format(colour_a, colour_b),
...
for index, band in enumerate("FNugriz"):
    setattr(
        NSA, "elpetro_absmag_{0}".format(band), HybridPropertyArrayIndex("elpetro_absmag", index)
    )
...
for colour_a, colour_b in itertools.combinations("FNugriz", 2):
    setattr(
        NSA,
        "elpetro_absmag_{0}_{1}".format(colour_a, colour_b),
...
def logmass(parameter):
    ...
    def mass(self):
        ...
        return math.log10(par) if par > 0.0 else 0.0

    @mass.expression
    def mass(cls):
        par = getattr(cls, parameter)
        return cast(case([(par > 0.0, func.log(par)), (par == 0.0, 0.0)]), Float)
...
def add_catalogue(classname, tablename, has_manga_target=None):
    ...
    if has_manga_target:
        relfks = [
            ("manga_target_pk", "mangasampledb.manga_target.pk"),
        ...
        new_relationalclass = ClassFactory(relationalClassName, relationalTableName, fks=relfks)
        globals()[relationalClassName] = new_relationalclass

        new_class.mangaTargets = relationship(
            MangaTarget,
            backref="{0}_objects".format(tablename),
