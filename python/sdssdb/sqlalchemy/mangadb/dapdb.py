...
class File(Base):
    ...
    def partner(self):
        ...
        return (
            session.query(File)
            .join(Structure, datadb.Cube, FileType)
            .filter(
                Structure.pk == self.structure.pk,
                datadb.Cube.pk == self.cube.pk,
                FileType.pk != self.filetype.pk,
            )
            .one()
        )
    ...
    def quality(cls):
        return (
            select([HeaderValue.value.cast(Integer)])
            .where(
                and_(
                    HeaderKeyword.pk == HeaderValue.header_keyword_pk,
...
class Hdu(Base):
    ...
    def header(self):
        ...
        session = database.Session.object_session(self)
        data = (
            session.query(HeaderKeyword.name, HeaderValue.value, HeaderValue.comment)
            .join(HeaderValue, HduToHeaderValue)
            .filter(
                HduToHeaderValue.header_value_pk == HeaderValue.pk,
                HduToHeaderValue.hdu_pk == self.pk,
            )
            .all()
        )
...
def HybridRatio(line1, line2):
    ...
    def hybridRatio(cls):
        ...
        return cast(
            case(
                [
                    (getattr(cls, line2) > 0.0, myline1 / getattr(cls, line2)),
                    (getattr(cls, line2) == 0.0, -999.0),
                ]
            ),
            Float,
        )
...
setattr(
    SpaxelAtts,
    "sii_to_ha",
    HybridRatio(("emline_gflux_sii_6718", "emline_gflux_sii_6732"), "emline_gflux_ha_6564"),
)
...
class ModelCube(Base):
    ...
    def get3DCube(self, extension="flux"):
        ...
        session = database.Session.object_session(self)
        spaxels = (
            session.query(getattr(ModelSpaxel, extension))
            .filter(ModelSpaxel.modelcube_pk == self.pk)
            .order_by(ModelSpaxel.x, ModelSpaxel.y)
            .all()
        )
...
def define_relations():
    ...
    Hdu.header_values = relationship(
        HeaderValue, secondary=HduToHeaderValue.__table__, backref="hdus"
    )
    ...
    Structure.template_kin = relationship(
        Template, foreign_keys=[Structure.template_kin_pk], backref="structures_kin"
    )
    Structure.template_pop = relationship(
        Template, foreign_keys=[Structure.template_pop_pk], backref="structures_pop"
    )

    # add foreign key relationships on (Clean)SpaxelProp classses to File
    spaxel_tables = {
        k: v for k, v in locals().items() if "SpaxelProp" in k or "CleanSpaxelProp" in k
    }
