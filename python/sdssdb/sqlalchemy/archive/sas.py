...
class Directory(Base):
    ...
    def children(self):
        ...
        realc = (
            session.query(Directory)
            .filter(
                Directory.tree == self.tree,
                Directory.location.ilike(self.location + "%"),
                Directory.nested == self.nested + 1,
            )
            .all()
        )
...
def define_relations():
    ...
    SymlinkDirectory.real_directory = relationship(
        Directory,
        backref="linked_symlink_directories",
        primaryjoin=("and_(SymlinkDirectory.real_directory_id==Directory.id)"),
    )

    SymlinkDirectory.tree = relationship(
        Tree,
        backref="symlink_directories",
    ...
    SymlinkFile.root = relationship(
        Root, backref="symlink_files", foreign_keys="SymlinkFile.root_id"
    )

    SymlinkFile.tree = relationship(
        Tree,
        backref="symlink_files",
