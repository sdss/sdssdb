...
class SDSS_DR14_APOGEE_Star(DeprecatedModel):
    ...
    visits = ManyToManyField(
        SDSS_DR14_APOGEE_Visit, through_model=_APOGEE_Star_Visit_Deferred, backref="stars"
    )
...
class SDSS_DR14_APOGEE_Star_Visit(DeprecatedModel):
    apstar = ForeignKeyField(SDSS_DR14_APOGEE_Star, backref="+", lazy_load=False)

    visit = ForeignKeyField(SDSS_DR14_APOGEE_Visit, backref="+", lazy_load=False)
...
class SDSS_DR14_ASCAP_Star(DeprecatedModel):
    apstar_id = TextField(primary_key=True)
    apstar = ForeignKeyField(SDSS_DR14_APOGEE_Star, backref="ascap_stars")
...
class SDSS_DR14_Cannon_Star(DeprecatedModel):
    ...
    def apstars(self):
        ...
        return SDSS_DR14_APOGEE_Star.select().where(
            SDSS_DR14_APOGEE_Star.apogee_id == self.apogee_id
        )
...
class Legacy_Survey_DR8_Lite(DeprecatedModel):
    ...
    ls = ForeignKeyField(Legacy_Survey_DR8, column_name="ls_id", backref="+")
    ...
    gaia = ForeignKeyField(
        Gaia_DR2, column_name="gaia_sourceid", object_id_name="gaia_sourceid", backref="+"
    )
...
class TwoMassPSC_Lite(DeprecatedModel):
    ...
    twomass_psc = ForeignKeyField(TwoMassPSC, column_name="pts_key", backref="+")
...
class TIC_v8_Lite(DeprecatedModel):
    ...
    tycho2 = ForeignKeyField(
        Tycho2, field="designation", column_name="tyc", object_id_name="tyc", backref="+"
    )

    twomass_psc = ForeignKeyField(
        TwoMassPSC, field="designation", column_name="twomass_psc", backref="+"
    )

    photoobj = ForeignKeyField(
        SDSS_DR13_PhotoObj, field="objid", column_name="sdss", object_id_name="sdss", backref="+"
    )

    gaia = ForeignKeyField(
        Gaia_DR2, field="source_id", column_name="gaia_int", object_id_name="gaia_int", backref="+"
    )
