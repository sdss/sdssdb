...
class Design(TargetdbBase):
    field = ForeignKeyField(column_name="field_pk", field="pk", model=Field, null=True)
    exposure = IntegerField(null=True)
    pk = AutoField()
    design_mode_pk = ForeignKeyField(
        column_name="design_mode_pk", field="label", model=DesignMode, null=True
    )
...
class Positioner(TargetdbBase):
    ...
    status = ForeignKeyField(
        column_name="positioner_status_pk", field="pk", model=PositionerStatus
    )
    info = ForeignKeyField(column_name="positioner_info_pk", field="pk", model=PositionerInfo)
...
class Target(TargetdbBase):
    catalogid = ForeignKeyField(
        column_name="catalogid", model=catalogdb.Catalog, field="catalogid"
    )
...
class Assignment(TargetdbBase):
    design = ForeignKeyField(Design, column_name="design_pk", field="pk")
    instrument = ForeignKeyField(Instrument, column_name="instrument_pk", field="pk")
    pk = AutoField()
    positioner = ForeignKeyField(Positioner, column_name="positioner_pk", field="pk")
...
class PlateToCatalog(TargetdbBase):
    pk = AutoField()
    plate_id = ForeignKeyField(column_name="plate_id", field="plate_id", model=Plate, null=False)
    catalogid = ForeignKeyField(
        column_name="catalogid", field="catalogid", model=catalogdb.Catalog, null=False
    )
...
class RsRunToSdssID(TargetdbBase):
    ...
    sdss_id = ForeignKeyField(
        column_name="sdss_id", field="sdss_id", model=catalogdb.SDSS_ID_flat, null=False
    )
