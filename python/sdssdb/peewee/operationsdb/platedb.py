...
class Design(OperationsDBModel):
    ...
    def get_value_for_field(self, field):
        ...
        return (
            DesignValue.select()
            .where((DesignValue.design == self) & (DesignValue.field == design_field))
            .first()
        )
...
class Tile(OperationsDBModel):
    pk = PrimaryKeyField()
    id = IntegerField()
    tile_status = ForeignKeyField(
        column_name="tile_status_pk", model=TileStatus, backref="tiles", field="pk"
    )
...
class Plate(OperationsDBModel):
    ...
    comment = TextField(null=True)
    current_survey_mode = ForeignKeyField(
        column_name="current_survey_mode_pk", null=True, model=SurveyMode, field="pk"
    )
    design = ForeignKeyField(
        column_name="design_pk", null=True, model=Design, backref="plates", field="pk"
    )
    ...
    plate_location = ForeignKeyField(
        column_name="plate_location_pk", model=PlateLocation, field="pk"
    )
    plate_run = ForeignKeyField(column_name="plate_run_pk", null=True, model=PlateRun, field="pk")
    rerun = TextField(null=True)
    temperature = FloatField(null=True)
    tile_id = IntegerField(column_name="tile_id", null=True)
    tile = ForeignKeyField(
        column_name="tile_pk", null=True, model=Tile, backref="plates", field="pk"
    )
    surveys = ManyToManyField(
        model=Survey, through_model=PlateSurveyThroughModel, backref="plates"
    )
    statuses = ManyToManyField(
        model=PlateStatus, through_model=PlateStatusThroughModel, backref="plates"
    )
...
class Plugging(OperationsDBModel):
    cartridge = ForeignKeyField(
        column_name="cartridge_pk", null=True, model=Cartridge, backref="pluggings", field="pk"
    )
    fscan = IntegerField(column_name="fscan_id", null=True)
    fscan_mjd = IntegerField(null=True)
    name = TextField(null=True)
    pk = PrimaryKeyField()
    plate = ForeignKeyField(column_name="plate_pk", model=Plate, backref="pluggings", field="pk")
    status = ForeignKeyField(
        column_name="plugging_status_pk", model=PluggingStatus, field="pk", backref="pluggings"
    )

    instruments = ManyToManyField(
        model=Instrument, through_model=PluggingInstrumentDeferred, backref="pluggings"
    )
...
class ActivePlugging(OperationsDBModel):
    pk = PrimaryKeyField()
    plugging = ForeignKeyField(
        column_name="plugging_pk",
        model=Plugging,
...
class BossPluggingInfo(OperationsDBModel):
    ...
    pk = PrimaryKeyField()
    plugging = ForeignKeyField(
        column_name="plugging_pk", model=Plugging, backref="boss_plugging_info", field="pk"
    )
...
class Camera(OperationsDBModel):
    instrument = ForeignKeyField(
        column_name="instrument_pk", null=True, model=Instrument, backref="cameras", field="pk"
    )
    label = TextField(null=True)
    pk = PrimaryKeyField()
...
class BossSn2Threshold(OperationsDBModel):
    camera = ForeignKeyField(
        column_name="camera_pk", model=Camera, backref="boss_sn2_thresholds", field="pk"
    )
...
class Pointing(OperationsDBModel):
    ...
    design = ForeignKeyField(
        column_name="design_pk", model=Design, backref="pointings", field="pk"
    )
    pk = PrimaryKeyField()
...
class PlatePointing(OperationsDBModel):
    ...
    pk = PrimaryKeyField()
    plate = ForeignKeyField(
        column_name="plate_pk", null=True, model=Plate, backref="plate_pointings", field="pk"
    )
    pointing_name = CharField(null=False)
    pointing = ForeignKeyField(
        column_name="pointing_pk", null=True, model=Pointing, field="pk", backref="plate_pointings"
    )
    priority = IntegerField()

    class Meta:
        db_table = "plate_pointing"
        indexes = ((("plate_pk", "pointing_name"), True),)
        schema = "platedb"
...
class Observation(OperationsDBModel):
    ...
    observation_status = ForeignKeyField(
        column_name="observation_status_pk",
        model=ObservationStatus,
        backref="observations",
        field="pk",
    )
    pk = PrimaryKeyField()
    plate_pointing = ForeignKeyField(
        column_name="plate_pointing_pk",
        null=True,
        model=PlatePointing,
        backref="observtions",
        field="pk",
    ...
    plugging = ForeignKeyField(
        column_name="plugging_pk", null=True, model=Plugging, backref="observations", field="pk"
    )
...
class Exposure(OperationsDBModel):
    camera = ForeignKeyField(
        column_name="camera_pk", null=True, model=Camera, backref="exposures", field="pk"
    )
    comment = TextField(null=True)
    exposure_flavor = ForeignKeyField(
        column_name="exposure_flavor_pk", null=True, model=ExposureFlavor, field="pk"
    )
    exposure_no = IntegerField(null=True)
    exposure_status = ForeignKeyField(
        column_name="exposure_status_pk", model=ExposureStatus, field="pk"
    )
    exposure_time = FloatField(null=True)
    observation = ForeignKeyField(
        column_name="observation_pk", null=True, model=Observation, backref="exposures", field="pk"
    )
    pk = PrimaryKeyField()
    start_time = FloatField(null=True)
    survey_mode = ForeignKeyField(
        column_name="survey_mode_pk", null=True, model=SurveyMode, field="pk"
    )
    survey = ForeignKeyField(
        column_name="survey_pk", null=True, model=Survey, backref="exposures", field="pk"
    )

    class Meta:
        db_table = "exposure"
        indexes = ((("exposure_no", "survey_pk"), True),)
        schema = "platedb"


class CameraFrame(OperationsDBModel):
    camera = ForeignKeyField(
        column_name="camera_pk", model=Camera, backref="camera_frames", field="pk"
    )
    comment = TextField(null=True)
    exposure = ForeignKeyField(
        column_name="exposure_pk", model=Exposure, backref="camera_frames", field="pk"
    )
    pk = PrimaryKeyField()
...
class CmmMeas(OperationsDBModel):
    ...
    pk = PrimaryKeyField()
    plate = ForeignKeyField(
        column_name="plate_pk", null=True, model=Plate, backref="cmm_measurements", field="pk"
    )
...
class DesignValue(OperationsDBModel):
    field = ForeignKeyField(
        column_name="design_field_pk", null=True, model=DesignField, field="pk"
    )
    design = ForeignKeyField(
        column_name="design_pk", null=True, model=Design, backref="values", field="pk"
    )
    pk = PrimaryKeyField()
    ...
    def __repr__(self):
        return "<DesignValue: pk={0}, design={1}, field={2}, value={3}>".format(
            self.pk, self.design_pk, self.field.label, self.value
        )
...
class ExposureHeaderValue(OperationsDBModel):
    comment = TextField(null=True)
    exposure_header_keyword = ForeignKeyField(
        column_name="exposure_header_keyword_pk", model=ExposureHeaderKeyword, field="pk"
    )
    exposure = ForeignKeyField(
        column_name="exposure_pk", model=Exposure, backref="exposure_header_values", field="pk"
    )
    index = IntegerField()
    pk = PrimaryKeyField()
    value = TextField()
...
class PlPlugmapM(OperationsDBModel):
    ...
    pk = PrimaryKeyField()
    plugging = ForeignKeyField(
        column_name="plugging_pk", null=True, model=Plugging, backref="plplugmapms", field="pk"
    )
...
class PlateHolesFile(OperationsDBModel):
    ...
    pk = PrimaryKeyField()
    plate = ForeignKeyField(
        column_name="plate_pk", model=Plate, backref="plate_hole_files", field="pk"
    )
...
class PlateHole(OperationsDBModel):
    ...
    object_type = ForeignKeyField(
        column_name="object_type_pk", null=True, model=ObjectType, field="pk"
    )
    pk = PrimaryKeyField()
    plate_hole_type = ForeignKeyField(
        column_name="plate_hole_type_pk", null=True, model=PlateHoleType, field="pk"
    )
    plate_holes_file = ForeignKeyField(
        column_name="plate_holes_file_pk",
        null=True,
...
class Fiber(OperationsDBModel):
    ...
    pk = PrimaryKeyField()
    pl_plugmap_m = ForeignKeyField(column_name="pl_plugmap_m_pk", model=PlPlugmapM, field="pk")
    plate_hole = ForeignKeyField(
        column_name="plate_hole_pk", model=PlateHole, backref="fibers", field="pk"
    )
...
class Gprobe(OperationsDBModel):
    cartridge = ForeignKeyField(
        column_name="cartridge_pk", null=True, model=Cartridge, backref="gprobes", field="pk"
    )
...
class HoleMeas(OperationsDBModel):
    cmm_meas = ForeignKeyField(column_name="cmm_meas_pk", null=True, model=CmmMeas, field="pk")
    ...
    pk = PrimaryKeyField()
    plate_hole = ForeignKeyField(
        column_name="plate_hole_pk",
        null=True,
        model=PlateHole,
        backref="hole_measurements",
        field="pk",
    )
...
class PlateCompletionStatusHistory(OperationsDBModel):
    ...
    pk = PrimaryKeyField()
    plate_completion_status = ForeignKeyField(
        column_name="plate_completion_status_pk",
        model=PlateCompletionStatus,
    ...
    plate = ForeignKeyField(
        column_name="plate_pk", model=Plate, backref="plate_completion_status_history", field="pk"
    )
...
class PlateInput(OperationsDBModel):
    comment = TextField(null=True)
    design = ForeignKeyField(
        column_name="design_pk", null=True, model=Design, backref="inputs", field="pk"
    )
...
class PlateToPlateStatus(OperationsDBModel):
    pk = PrimaryKeyField()
    plate = ForeignKeyField(column_name="plate_pk", null=True, model=Plate, field="pk")
    plate_status = ForeignKeyField(
        column_name="plate_status_pk", null=True, model=PlateStatus, field="pk"
    )

    class Meta:
        db_table = "plate_to_plate_status"
        indexes = ((("plate_status_pk", "plate_pk"), True),)
        schema = "platedb"
...
class PlateToSurvey(OperationsDBModel):
    pk = PrimaryKeyField()
    plate = ForeignKeyField(column_name="plate_pk", null=True, model=Plate, field="pk")
    survey = ForeignKeyField(column_name="survey_pk", null=True, model=Survey, field="pk")

    class Meta:
        db_table = "plate_to_survey"
        indexes = ((("plate_pk", "survey_pk"), True),)
        schema = "platedb"
...
class PluggingToInstrument(OperationsDBModel):
    instrument = ForeignKeyField(
        column_name="instrument_pk", null=True, model=Instrument, field="pk"
    )
    pk = PrimaryKeyField()
    plugging = ForeignKeyField(column_name="plugging_pk", null=True, model=Plugging, field="pk")

    class Meta:
        db_table = "plugging_to_instrument"
        indexes = ((("plugging_pk", "instrument_pk"), True),)
        schema = "platedb"
...
class ProfTolerances(OperationsDBModel):
    ...
    r5_low = FloatField()
    survey = ForeignKeyField(
        column_name="survey_pk", model=Survey, backref="prof_tolerances", field="pk"
    )
...
class Profilometry(OperationsDBModel):
    ...
    pk = PrimaryKeyField()
    plugging = ForeignKeyField(
        column_name="plugging_pk", model=Plugging, backref="profilometries", field="pk"
    )
    prof_tolerances = ForeignKeyField(
        column_name="prof_tolerances_pk",
        model=ProfTolerances,
        backref="profilometries",
        field="pk",
    )
    timestamp = DateTimeField()
...
class ProfMeasurement(OperationsDBModel):
    number = IntegerField()
    pk = PrimaryKeyField()
    profilometry = ForeignKeyField(column_name="profilometry_pk", model=Profilometry, field="pk")
...
class TileStatusHistory(OperationsDBModel):
    ...
    pk = PrimaryKeyField()
    tile = ForeignKeyField(
        column_name="tile_pk", model=Tile, backref="tile_status_history", field="pk"
    )
    tile_status = ForeignKeyField(
        column_name="tile_status_pk", model=TileStatus, backref="tile_status_history", field="pk"
    )
    timestamp = DateTimeField()
