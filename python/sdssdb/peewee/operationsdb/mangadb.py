...
class Plate(OperationsDBModel):
    ...
    pk = PrimaryKeyField()
    platedb_plate = ForeignKeyField(
        column_name="platedb_plate_pk", null=False, model=PlatedbPlate, field="pk", unique=True
    )
...
class DataCube(OperationsDBModel):
    ...
    pk = PrimaryKeyField()
    platedb_plate = ForeignKeyField(
        column_name="plate_pk", null=True, model=PlatedbPlate, field="pk"
    )
...
class Set(OperationsDBModel):
    ...
    status = ForeignKeyField(
        column_name="set_status_pk", null=True, model=SetStatus, field="pk", backref="sets"
    )
...
class Exposure(OperationsDBModel):
    comment = TextField(null=True)
    data_cube = ForeignKeyField(
        column_name="data_cube_pk", null=True, model=DataCube, backref="exposures", field="pk"
    )
    dither_dec = FloatField(null=True)
    dither_position = UnknownField(null=True)  # ARRAY
    dither_ra = FloatField(null=True)
    status = ForeignKeyField(
        column_name="exposure_status_pk",
        null=True,
    ...
    pk = PrimaryKeyField()
    platedb_exposure = ForeignKeyField(
        column_name="platedb_exposure_pk",
        null=True,
    ...
    set = ForeignKeyField(
        column_name="set_pk", null=True, model=Set, field="pk", backref="exposures"
    )
...
class ExposureToDataCube(OperationsDBModel):
    data_cube = ForeignKeyField(column_name="data_cube_pk", null=True, model=DataCube, field="pk")
    exposure = ForeignKeyField(column_name="exposure_pk", null=True, model=Exposure, field="pk")
    pk = PrimaryKeyField()
...
class Sn2Values(OperationsDBModel):
    ...
    exposure = ForeignKeyField(
        column_name="exposure_pk", null=True, model=Exposure, backref="sn2_values", field="pk"
    )
...
class Spectrum(OperationsDBModel):
    data_cube = ForeignKeyField(
        column_name="data_cube_pk", null=True, model=DataCube, backref="spectrums", field="pk"
    )
    exposure = ForeignKeyField(
        column_name="exposure_pk", null=True, model=Exposure, backref="spectrums", field="pk"
    )
